...
def create_rnn_cell(num_units, num_layers, keep_prob, RNN_type, activation_fn): 
    ...
    for _ in range(num_layers):
        if RNN_type == 'GRU':
            cell = tf.contrib.rnn.GRUCell(num_units, activation=activation_fn, reuse=tf.AUTO_REUSE)
        elif RNN_type == 'LSTM':
            cell = tf.contrib.rnn.LSTMCell(num_units, activation=activation_fn, state_is_tuple=True, reuse=tf.AUTO_REUSE)
