function [rec, cum_eff, cum_tox, cum_s, typeI, typeII, q_mse] = ...
    C3T_Budget_E(T, B, S, K, pats, arr_rate, tox_thre, eff_thre, p_true, q_true, opt_ind)
% C3T_Budget_E  Simulated trial with C3T-Budget-E
%   Input
%     T        : time-horizon
%     B        : budget
...
while t <= T && b < B
    ...
    if s_arrive(H(t)) < K + 1 % initialize
        ...
        X(t) = binornd(1, q_true(curr_s, I(t)));
        eff_temp = eff_temp + X(t);
        Y(t) = binornd(1, p_true(I(t)));
        ...
        while tmp_dist <= rho && tilde_s < S
            tilde_s = tilde_s + 1;
            tmp_dist = tmp_dist + table(tilde_s, 3);
        end
        tilde_s = tilde_s - 1;
        ...
        dose_prob = zeros(S, 1);
        if tilde_s > 0
            dose_prob(table(1:tilde_s, 2)) = 1;
            dose_prob(table(tilde_s+1, 2)) = (rho - sum(table(1:tilde_s, 3))) / table(tilde_s+1, 3);
            if tilde_s + 2 <= S
                dose_prob(table(tilde_s+2:end, 2)) = 0;
            end
        end
        ...
        if dose_prob(curr_s) >= rand() % take dose
            ...
            X(t) = binornd(1, q_true(curr_s, I(t)));
            eff_temp = eff_temp + X(t);
            Y(t) = binornd(1, p_true(curr_s, I(t)));
            tox_temp = tox_temp + Y(t);
