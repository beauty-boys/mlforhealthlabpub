...
class GRV_B(object):
    def __init__(self, x_list,a_list, time, m_rnn_hidden=32, dtr_rnn_hidden=32, m_hidden=32,
                 dtr_hidden=32, lr=0.001,Lambda =1.0, maximize = True):

        self.start, self.time,  self.Lambda,self.lr = True, time, Lambda, lr
    ...
    def d_part(self, output_dtr, data_slice):
        ...
        x_t, _, _, _ = data_slice[self.time-1]

        embed_dtr_t = hist_embedding(output_dtr, self.time-1)
        self.prob_t, self.d_t = self.dtr_net(embed_dtr_t, x_t)
        d_t = tf.transpose(tf.expand_dims(self.d_t,axis=2),[0,2,1])
        if self.time<self.max_length:
            self.d_matrix = tf.concat([d_t,self.d_t_T],axis=1)
        else:
            self.d_matrix = d_t
    ...
    def v_part(self, output_g, output_v, data_slice):
        ...
        for step in range(self.time-1,self.max_length)[::-1]:
            ...
            delta_tt = tf.squeeze(tf.slice(self.d_matrix, [0, step - (self.time - 1), 0], [-1, 1, -1]), axis=1)
            delta_tt = tf.reduce_sum(a_t * delta_tt, axis=1)
            self.R += tf.reduce_mean(tf.square(y_t - mu_tt_d - self.m_params['epsilon_' + str(self.time-1)] *delta_tt/ tf.reduce_sum(g_t * a_t, axis=1)))
        ...
        step, prob_cum, delta_cum = self.time - 1, {}, {}
        treatment_t_T = tf.slice(self.treatments, [0, step, 0], [-1, -1, -1])
        delta = tf.reduce_sum(self.d_matrix * treatment_t_T , axis=2)
        ...
        for r in range(step, self.max_length):
            for s in range(step, r - 1):
                zeta_s_r_t = tf.reduce_mean(tf.square(self.mu_d_e[str(step) + '_' + str(s + 1) + '_' + str(r)] - self.mu_d_e[str(step) + '_' + str(s) + '_' + str(r)]))
                self.R += zeta_s_r_t
    ...
    def losses(self):
        ...
        step = self.time - 1
        treatment_t_T = tf.slice(self.treatments, [0, step, 0], [-1, -1, -1])
        delta_t_T = tf.cumprod(tf.reduce_sum(self.d_matrix * treatment_t_T, axis=2), axis=1)

        self.weight__ = self.mu_d[str(step)] + self.m_params['epsilon_' + str(step)] \
                        * tf.cumsum(tf.cumprod(tf.reduce_sum(self.d_matrix * treatment_t_T, axis=2), axis=1) / tf.cumprod(self.prob_matrix,  axis=1), axis=1)

        #Remove delta_t_T if the treatment rules do not depend on the treatments in the history and the treatment has no influence on the future covariates
        self.loss_dtr_sum = tf.reduce_sum(delta_t_T * self.weight__, axis=1)
    ...
    def fit(self, rnn_X, treatments, rnn_Y, batch_size=50, num_initial=500, num_d=100, num_v=5, d_t_T=None):
        ...
        for j in range(num_d):
            ...
            for i in range(num_batch):
                ...
                feed_input_2 = {self.rnn_x: batch_rnn_x,
                                ...
                                self.y: batch_y}
                if self.time < self.max_length:
                   batch_d = d_t_T[batch_index, :, :]
                   feed_input_2.update({self.d_t_T:batch_d})

                self.sess.run([self.loss_dtr_sum, self.opt_dtr_sum],feed_dict=feed_input_2)
            ...
            for k in range(num_v):
                ...
                for i in range(num_batch):
                    ...
                    if self.time < self.max_length:
                       batch_d = d_t_T[batch_index, :, :]
                       feed_input_1.update({self.d_t_T:batch_d})

                    self.sess.run([self.loss_v_sum, self.opt_v_sum], feed_dict=feed_input_1)
    ...
    def predict(self, rnn_X, treatments):
        ...
        prob_t, _ = self.sess.run([self.prob_t, self.d_t], feed_dict={self.rnn_x: rnn_X, self.treatments: treatments})
        d_t = np.zeros_like(prob_t)

        for i in range(np.shape(rnn_X)[0]):
            index = np.argmax(prob_t[i, :])
            d_t[i, index] = 1.0
    ...
    def initial(self):
        ...
        self.rnn_input = tf.concat([self.treatments, self.rnn_x], axis=2)

        if self.time<self.max_length:
            self.d_t_T = tf.placeholder(tf.float32, [None, self.max_length-self.time, self.n_treatments])
        ...
        self.m_params['epsilon_' +str(self.time-1)]  = tf.Variable(tf.random_normal([1], stddev=0.1),name='value_epsilon')
        self.m_params['epsilon_' + str(self.time - 1)] = self.m_params['epsilon_' +str(self.time-1)]/10000
        ...
        with tf.variable_scope("rnn_g"):
            output_g,_ = rnn.dynamic_rnn(cell=self.lstm_cell_g, inputs=self.rnn_input, dtype=tf.float32)

        with tf.variable_scope("rnn_value"):
            output_v,_ = rnn.dynamic_rnn(cell=self.lstm_cell_m, inputs=self.rnn_input, dtype=tf.float32)

        with tf.variable_scope("rnn_dtr"):
            output_dtr,_ = rnn.dynamic_rnn(cell=self.lstm_cell_dtr, inputs=self.rnn_input, dtype=tf.float32)

        return output_g, output_v, output_dtr


    def save_network(self, data_name,exp_index, time, destroy=False):

        checkpoint_name = 'GRVB_' + data_name + '_exp_' + str(exp_index) + '_step_' + str(time)
        model_dir = "GRVB_model/" + data_name + '/exp_' + str(exp_index) + "/step_" + str(time)
    ...
    def load_network(self, data_name, exp_index, time):
